...
def pd_desurvey_straight(dfs, vhid, vx, vy, vz, vdepth, vbrg, vdip, vfrom, vto, xyz = None, downhole = False):
  ...
  # synonyms
  if not v_lut['hid']:
    v_lut['hid'] = pd_synonyms(dfs[0], ["hole", "hole_number", "furo", "hid", "bhid", "dhid"])
  if not v_lut['x']:
    v_lut['x'] = pd_synonyms(dfs[0], ["x", "xpt", "east", "easting", "leste"])
  if not v_lut['y']:
    v_lut['y'] = pd_synonyms(dfs[0], ["y", "ypt", "north", "northing", "norte"])
  if not v_lut['z']:
    v_lut['z'] = pd_synonyms(dfs[0], ["z", "zpt", "level", "cota", "elev"])
  if not v_lut['depth']:
    v_lut['depth'] = pd_synonyms(dfs[1], ["depth", "prof", "to", "ate"])
  if not v_lut['brg']:
    v_lut['brg'] = pd_synonyms(dfs[1], ["brg", "azimuth", "azim", "azi"])
  if not v_lut['dip']:
    v_lut['dip'] = pd_synonyms(dfs[1], ["dip", "inclin"])
  if not v_lut['from']:
    v_lut['from'] = pd_synonyms(dfs[2], ["from", "de"])
  if not v_lut['to']:
    v_lut['to'] = pd_synonyms(dfs[2], ["to", "ate"])
...
def db_desurvey_straight(header, vhid, vx, vy, vz, survey, vdepth, vbrg, vdip, assay, vfrom, vto, output, display = False, downhole = False):
  xyz = ['mid_x','mid_y','mid_z']
  dfs = []
  dfs.append(pd_load_dataframe(header))
  dfs.append(pd_load_dataframe(survey))
  if assay:
    dfs.append(pd_load_dataframe(assay))
